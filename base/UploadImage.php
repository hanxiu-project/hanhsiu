...
if (isset($_FILES['upload']) && strlen($_FILES['upload']['name']) > 1) {
    ...
    if (in_array($type, $imgset['type'])) {
        ...
        if (isset($width) && isset($height)) {
            if ($width > $imgset['maxwidth'] || $height > $imgset['maxheight']) {
                $re .= '\\n Width x Height = ' . $width . ' x ' . $height . ' \\n 上傳圖檔最大寬度 x 高度須為： ' . $imgset['maxwidth'] . ' x ' . $imgset['maxheight'];
            }

            if ($width < $imgset['minwidth'] || $height < $imgset['minheight']) {
                $re .= '\\n Width x Height = ' . $width . ' x ' . $height . '\\n 上傳圖檔最小寬度 x 高度須為：  ' . $imgset['minwidth'] . ' x ' . $imgset['minheight'];
            }

            if ($_FILES['upload']['size'] > $imgset['maxsize']) {
                $re .= '\\n 上傳圖檔大小須小於： ' . ' 2MB.';
            }
        }
    } else {
        $re .= '上傳圖檔： ' . $_FILES['upload']['name'] . '須為jpg、jpeg、bmp、gif或png格式';
    }
    ...
    if ($re == '') {
        if (move_uploaded_file($_FILES['upload']['tmp_name'], $uploadpath)) {
            ...
            $url = '/hanhsiu/uploads/posts/' . $f_name;
            $msg = F_NAME . '.' . $type . ' 成功上傳';
