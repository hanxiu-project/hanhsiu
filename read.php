            ...
            <div class="container main-wrap">
                    ...
                    <div class="close-wrap site1">
                        ...
                        <div>
                            ...
                            <div class="read-mode__ctrl">
                                <select class="form-select mb-2" id="bigreadtype_select" onchange="changeBigType(this.value)">
                                    <?php
                                ...
                                <select class="form-select mb-2" id="smallreadtype_select"></select>
                ...
                <div class="main-wrap__right">
                    ...
                    ?>
                        ...
                        <h4 class="heading-style__2 mb-4" name="readtype_id" id=<?php echo "$_GET[tid]" ?>><?php echo "$r_type[typename]" ?></h4>
                    ...
                    if (isset($_GET["stid"])) {
                        ...
                        $data = mysqli_num_rows($result_type);
                    ?>
                        <h4 class="heading-style__2 mb-4" name="readtype_id" id="<?php echo "$r_type[t_id]" ?>"><?php echo "$r_type[typename]"; ?>
                            <span class="tline" name="read_smalltype_id" id="<?php echo "$r_type[st_id]" ?>"><?php echo "$r_type[s_typename]"; ?></span>
                        </h4>
...
<script type="text/javascript">
    function opData() {
        var currentlyTId = document.getElementsByName('readtype_id')[0].id;
        var optionLength = document.getElementById("bigreadtype_select").options.length;

        for (var i = 0; i < optionLength; i++) {
            if (document.getElementById("bigreadtype_select").options[i].value == currentlyTId) {
                document.getElementById("bigreadtype_select").options[i].selected = true;
            }
        }
    ...
    function changeBigType(index) {
        var chapterSelect = document.getElementById("smallreadtype_select");

        $.ajax({
            url: "deal.php",
            method: "POST",
            data: {
                SelectedReadType: index
            },
    ...
    function keepAID() {
        var currentlyVId = document.getElementsByName("read_smalltype_id")[0].id;
        var optionLength = document.getElementById("smallreadtype_select").options.length;
        if (optionLength != 0) {
            for (var i = 0; i < optionLength; i++) {
                if (document.getElementById("smallreadtype_select").options[i].value == currentlyVId) {
                    document.getElementById("smallreadtype_select").options[i].selected = true;
                }
            }
        }
    }
    ...
    function confirm() {
        var optionLength = document.getElementById("smallreadtype_select").options.length;
        var optionBigLength = document.getElementById("bigreadtype_select").options.length;
        for (var i = 0; i < optionBigLength; i++) {
            if (document.getElementById("bigreadtype_select").options[i].selected == true) {
                var currentlyTId = document.getElementById("bigreadtype_select").options[i].value
            }
        }
        if (optionLength != 0) {
            for (var i = 0; i < optionLength; i++) {
                if (document.getElementById("smallreadtype_select").options[i].selected == true) {
                    var currentlySId = document.getElementById("smallreadtype_select").options[i].value;
                }
            }
        }
        ...
        if (optionLength == 0) {
            ...
            for (var i = 0; i < optionLength; i++) {
                if (document.getElementById("smallreadtype_select").options[i].value == currentlySId) {
                    window.open('read.php?stid=' + currentlySId, '_self');
                }
            }
        }
    }
